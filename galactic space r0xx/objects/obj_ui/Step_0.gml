dead = global.player_inst.state == STATES.dead;

//decrease flashes alpha to 0 overtime
for(var i=0; i < 6; i++)
	list_flash_a[|i] = approach(list_flash_a[|i],0,0.05);
	
hp_text_a = approach(hp_text_a,0,0.05);

trans_prec = lerp(trans_prec,dead,0.01 + 0.02 * keyboard_check(vk_space));
trans_prec = approach(trans_prec,dead,0.0001);
...
if(bars_anim_prec != -1)
{
	...
	if(bars_anim_prec == 5.9)
	{
		...
		//my_effect2 = create_effect(spr_bars_anim_sparks,0.8,0,_res[0]/2,_res[1] - 14 + 20 * arr_curves[UI_ELEM.bars]);
		my_effect.image_alpha = 0;
		//my_effect2.image_alpha = 0;
